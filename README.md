# PubNub 3.6.5 for iOS 5.1+ (iPhone, iPad, iPod)
...
+ (void)subscribeOnChannels:(NSArray *)channels withClientState:(NSDictionary *)clientState
 andCompletionHandlingBlock:(PNClientChannelSubscriptionHandlerBlock)handlerBlock;
```
Each subscription method has designated methods, to add a presence flag, client state and/or add a handling block.  
...
[PubNub subscribeOnChannels:[PNChannel channelsWithName:@"iosdev" shouldObservePresence:YES]
...
+ (PNMessage *)sendMessage:(id)message applePushNotification:(NSDictionary *)apnsPayload 
   googleCloudNotification:(NSDictionary *)gcmPayload toChannel:(PNChannel *)channel 
                compressed:(BOOL)shouldCompressMessage withCompletionBlock:(PNClientMessageProcessingBlock)success;  
+ (PNMessage *)sendMessage:(id)message applePushNotification:(NSDictionary *)apnsPayload
   googleCloudNotification:(NSDictionary *)gcmPayload toChannel:(PNChannel *)channel compressed:(BOOL)shouldCompressMessage 
            storeInHistory:(BOOL)shouldStoreInHistory;  
+ (PNMessage *)sendMessage:(id)message applePushNotification:(NSDictionary *)apnsPayload 
   googleCloudNotification:(NSDictionary *)gcmPayload toChannel:(PNChannel *)channel compressed:(BOOL)shouldCompressMessage 
            storeInHistory:(BOOL)shouldStoreInHistory withCompletionBlock:(PNClientMessageProcessingBlock)success;  
...
```
Also, if you want to send push notifications along with message which should be delivered to subscribers, you can use one of the following methods:  
```objc
+ (PNMessage *)sendMessage:(id)message applePushNotification:(NSDictionary *)apnsPayload toChannel:(PNChannel *)channel;
+ (PNMessage *)sendMessage:(id)message applePushNotification:(NSDictionary *)apnsPayload toChannel:(PNChannel *)channel
       withCompletionBlock:(PNClientMessageProcessingBlock)success;  
...
+ (PNMessage *)sendMessage:(id)message applePushNotification:(NSDictionary *)apnsPayload 
   googleCloudNotification:(NSDictionary *)gcmPayload toChannel:(PNChannel *)channel compressed:(BOOL)shouldCompressMessage 
            storeInHistory:(BOOL)shouldStoreInHistory withCompletionBlock:(PNClientMessageProcessingBlock)success;  
```
In case if you won't want to store certain message inside history storage, you can use of of methods with `storeInHistory` flag. Set it to `NO` and message which you will send won't be available in future through [history API](#history).

If `apnsPayload` or `gcmPayload` specified, only value stored in it will be delivered to device using APNS and GCM notification delivery services. Subscribers will receive composed message from content of values passed in `message`, `apnsPayload` and `gcmPayload`.  
For example:  
```objc
...
```
Subscribers on `iosdev` will receive next payload:
```json
{
    "pn_apns":{
        "aps":{
            "alert":"Someone sent array of strings"
            }
        },
    "pn_gcm":{
        "data":{
            "summary":"Someone sent array of strings"
            }
        },
    "pn_other":{
        "array":["of", "strings"], 
...
```
iOS devices will receive remote push notification with next payload:
```json
{
    "aps":{
        "alert":"Someone sent array of strings"
